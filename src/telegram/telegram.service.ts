...
export class TelegramService implements OnModuleInit {
  ...
  private async initializeBots() {
    try {
      ...
      config.bots.forEach((botConfig) => {
        this.logger.debug(
          `Adding bot ${botConfig.token.slice(0, 6)}... to load balancer`,
        );
  ...
  private async handleIncomingMessage(
    ...
    if (message.chat.type !== 'private') {
      this.logger.debug(
        `Skipping non-private message from chat ${message.chat.id} (type: ${message.chat.type})`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.debug(
        `Processing message from chat ${message.chat.id} using bot ${botToken}`,
      );
      this.logger.debug(
      ...
      const matchingChannel = config.channels.find((channel) => {
        ...
        this.logger.debug(
          `Checking channel ${channel.channelId} (${channel.description || 'no description'}): ${hasToken ? 'matches' : 'no match'}`,
        );
        return hasToken;
      });
      if (!matchingChannel) {
        this.logger.warn(
          `No matching channel found for bot token: ${botToken}`,
        );
      ...
      this.logger.debug(
        `Forwarding message ${message.message_id} to channel ${matchingChannel.channelId} (${matchingChannel.description || 'no description'})`,
      );
      ...
      try {
        ...
        this.logger.debug(
          `Successfully forwarded message ${message.message_id} using primary bot`,
        );
      } catch (error) {
        this.logger.warn(
          `Failed to forward message with original bot: ${error.message}`,
        );
        // Try with a different bot as fallback
        const fallbackBot = this.loadBalancer.getNextBot();
        if (fallbackBot !== bot) {
          this.logger.debug(
            `Attempting to forward message ${message.message_id} using fallback bot`,
          );
          ...
          this.logger.debug(
            `Successfully forwarded message ${message.message_id} using fallback bot`,
          );
        } else {
    ...
    try {
      ...
      switch (messageDto.type) {
        case MessageType.PHOTO:
          ...
          const photoResult = await bot.sendPhoto(
            channel.channelId,
            messageDto.mediaUrl,
            {
        ...
        case MessageType.VIDEO:
          ...
          const videoResult = await bot.sendVideo(
            channel.channelId,
            messageDto.mediaUrl,
            {
              caption: messageDto.message,
        ...
        default:
          const textResult = await bot.sendMessage(
            channel.channelId,
            messageDto.message,
          );
